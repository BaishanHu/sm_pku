...
namespace SM
{
  ...
  //Lanczos algorithm, MatGen has variable dim and method getME(i,j),getJ2(i,j)
  template<typename MatGen>
    ...
    public:
      ...
      //      typedef MatrixType MatrixAType;//may be sparse Matrix
      typedef SparseMatrix<Scalar> MatrixAType;
      
      LanczosIter(const MatGen & _matgen,const communicator& _world);//:matgen(_matgen),dim(0),totaldim(_matgen.dim),world(_world),restartdim(int)
	...
	clear();      
      ...
      int getMat(MatrixType & Mat,int _dim);
      ...
      void getEigVecs(MatrixType & eigVecs,int numStates);

      
      vector<VectorType> partV;//meaningful for rank 0-numOfparts-1
      VectorType partu;//the rest,meaningful for rank 0-numOfparts-1
      vector<VectorType> partrest;// every time restart, store the rest vector
      vector< vector<Scalar> > bandvals;//nonzero values on band
      ...
      const int restartdim;//every restartdim, the lanczos iteration restart
  ...
  template<typename MatGen>
//hubsh    LanczosIter<MatGen>::LanczosIter(const MatGen & _matgen,const communicator& _world):matgen(_matgen),dim(0),totaldim(_matgen.dim),world(_world),restartdim(100)
    LanczosIter<MatGen>::LanczosIter(const MatGen & _matgen,const communicator& _world):matgen(_matgen),dim(0),totaldim(_matgen.dim),world(_world),restartdim(1000)
    {
  ...
  template<typename MatGen>
    ...
    {
    std::ofstream fout("results_lanczos_monitor.dat");
      ...
      if(world.rank()==0)
	{
	  ...
	  fout<<"total number of non-zero A matrix elements: "<<numAnonZeros<<std::endl;
	  fout<<"total number of non-zero J2 matrix elements: "<<numJ2nonZeros<<std::endl;
	  fout<<"estimated memory: "<<(numAnonZeros+numJ2nonZeros)*sizeof(Scalar)/double(1<<30)<<"G\n";
	}
      else
	{
  ...
  template<typename MatGen>
      ...
      if(dim==0)
	{
	  ...
	  dim++;
      ...
      else if(dim%restartdim==0)//restart
      	{
      	  if(rank==0)
      	    {
      	      bandvals.push_back(vector<Scalar>());
      	      V0=VectorType::Random(totaldim);
      	      V0/=sqrt(V0.transpose()*V0);
      	    }
      	  if(rank<numOfparts)
      	    {
      	      partrest.push_back(partu);

      	      broadcast(diagcom,V0,0);
      	      int start=rank*partdim;
      	      int partrows=partdim;
      	      if(rank==numOfparts-1)
      		partrows=totaldim-rank*partdim;
	      partu=V0.segment(start,partrows);
	      ...
	      if(rank==0) norm=sqrt(norm);
	      broadcast(diagcom,norm,0);
	      partu/=norm;
      	      partV.push_back(partu);

	      for(int i=0;i<partrest.size()-1;i++)
		{
		  Scalar b;
		  udotv(partrest[i],partV[partV.size()-1],b);
		  if(rank==0)
		    {
		      bandvals[i].push_back(b);
		    }
		}	      
      	    }
      	  AmultiplyV(partA,partV[partV.size()-1],partu);
      	  if(rank<numOfparts)
      	    {
      	      Scalar a,b;
      	      udotv(partu,partV[partV.size()-1],a);
      	      udotv(partu,partV[partV.size()-2],b);
      	      broadcast(diagcom,a,0);
	      ...
	      /* for(int i=0;i<partrest.size()-1;i++) */
	      /* 	{ */
	      /* 	  int index=restartdim*(i+1)-1; */
	      /* 	  partu-=bandvals[i][partV.size()-1-index-2]*partV[index]; */
	      /* 	} */
      	    }
      	  dim++;
      	}
  ...
  template<typename MatGen>
    int LanczosIter<MatGen>::getMat(MatrixType & Mat,int _dim)
    {
      if(dim<_dim)
	{
	  while(next())
      ...
      if(rank==0)
	{
	  ...
	  for(int i=0;i<partrest.size();i++)
	    {
	      int start=restartdim*(i+1)-1;
	      for(int j=0;j<_dim-start-2;j++)
	  	{
	  	  Mat(start,start+2+j)=bandvals[i][j];
	  	  Mat(start+2+j,start)=bandvals[i][j];
	  	}
	    }
	}
      return _dim;
  ...
  template<typename MatGen>
    void LanczosIter<MatGen>::compute(VectorType & m_eivalues,int max_dim,bool yesvecs)
    {
      ...
      MatrixType Mat;
      int n=getMat(Mat,max_dim);
            
      if(rank==0)
	{
	  ComplexEigenSolver<MatrixType> es(Mat);
	  //SelfAdjointEigenSolver<MatrixType> es(Mat);
	  m_eivalues=es.eigenvalues();
	  Q=es.eigenvectors();
	  if(yesvecs)
	    {
	      for(int i=0;i<Q.cols();i++)
	  	{
	  	  Scalar norm=Q.col(i).transpose()*Q.col(i);
	  	  Q.col(i)/=sqrt(norm);
	  	}
